...
BEGIN
        ...
        IF @operation = 'Delete'
                ...
                SELECT GETDATE(), d.Date, d.Task, d.Comments, @operation
                FROM deleted d
        IF @operation = 'Insert'
                INSERT INTO WorkHoursLogs (ChangeDate, NewTaskDate, NewTask, NewHours, NewComments, Command)
                SELECT GETDATE(), i.Date, i.Task, i.Comments, @operation
                FROM inserted i
        IF @operation = 'Update'
                INSERT INTO WorkHoursLogs (ChangeDate, OldTaskDate, OldTask, OldHours, OldComments,
                        NewTaskDate, NewTask, NewHours, NewComments, Command)
                                SELECT GETDATE(), d.Date, d.Task, d.Comments, i.Date, i.Task,
                                       i.Comments, @operation
                                FROM deleted d, inserted i
END
GO
               
 
UPDATE WorkHours SET [Date] = DATEADD(MONTH, 3, GETDATE()) WHERE WorkHoursLogsId = 2
UPDATE WorkHours SET Comments = 'I`ve changed that thing and a log is in the WorkHOursLog' WHERE WorkHoursLogsId = 1
